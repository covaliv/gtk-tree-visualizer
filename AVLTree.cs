...
public class AVLTree<T> : Tree<T> where T : IComparable<T>
{
    ...
    private Node Insert(Node node, T value)
    {
        ...
        if (balance > 1)
        {
            ...
            else
            {
                updatesLog.AppendLine(
                    $"Left-right imbalance detected at node {node.Value}. Performing left-right rotation.");
        ...
        if (balance < -1)
        {
            ...
            {
                updatesLog.AppendLine(
                    $"Right-right imbalance detected at node {node.Value}. Performing left rotation.");
                return RotateLeft(node);
            }
            else
            {
                updatesLog.AppendLine(
                    $"Right-left imbalance detected at node {node.Value}. Performing right-left rotation.");
    ...
    private Node RotateRight(Node y)
    {
        ...
        updatesLog.AppendLine(
            $"Rotation right complete. New heights: Node {y.Value} height {y.Height}, Node {x.Value} height {x.Height}.");
    ...
    private Node RotateLeft(Node x)
    {
        ...
        updatesLog.AppendLine(
            $"Rotation left complete. New heights: Node {x.Value} height {x.Height}, Node {y.Value} height {y.Height}.");
    ...
    private Node Delete(Node node, T value)
    {
        ...
        if (balance > 1) // if the node is left-heavy
        {
            ...
            {
                updatesLog.AppendLine(
                    $"Left-heavy imbalance detected at node {node.Value}. Performing right rotation.");
        ...
        if (balance < -1) // if the node is right-heavy
        {
            ...
            {
                updatesLog.AppendLine(
                    $"Right-heavy imbalance detected at node {node.Value}. Performing left rotation.");
